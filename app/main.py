...
class SuperAIPlus:
    ...
    def get_smart_response(self, message: str, user_id: int) -> str:
        ...
        # 💬 ОБЩИЕ ВОПРОСЫ
        responses = {
            "привет": "🚀 Привет! Я SuperAi+ - твой умный помощник!",
            "как дела": "💫 Отлично! Работаю над интересными задачами. А у тебя?",
            "что ты умеешь": "🎯 Я умею: голосовые сообщения, анализ фото, декомпозицию целей, умные беседы и многое другое!",
        ...
        if "анализ" in message_lower:
            self.user_data[user_id]['neurons'] += 2
            return "🔍 Готов анализировать! Что именно вас интересует?"
        
        elif any(word in message_lower for word in ["цель", "задач", "план"]):
            return "🎯 Используйте декомпозитор целей: /decompose Ваша цель"
    ...
    try:
        ...
        if "voice" in update["message"]:
            ...
            if text.startswith("/start"):
                ...
                response = "🤖 **Помощь:** Используйте меню для доступа ко всем функциям!"
